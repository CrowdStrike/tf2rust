...
from nodes.inputLayerNode import InputLayerNode
...
def get_option_preferences(nodes_dict, computational_graph):
    ...
    # Remove layers that can be done in place, but doesnt have all the inputs set to mut
...
[build-dependencies]
bincode = "1.3.1"
ndarray = { version = "0.14.0" }
ndarray-npy = "0.7.1"
serde = { version = "1.0.117", features = ["derive"] }
tensorflow_layers = { package = "tensorflow-layers", version = "0.3.0" }
...
bincode = "1.3.1"
lazy_static = "1.4.0"
ndarray = { version = "0.14.0", features = ["serde-1"] }
serde = { version = "1.0.117", features = ["derive"] }
# json data format for serde
serde_json = "1.0.40"
tensorflow_layers = { package = "tensorflow-layers", version = "0.3.0" }

[target.'cfg(windows)'.dependencies]
windows = { version = "0.39.0", features = ["Win32_System_LibraryLoader", "Win32_Foundation"] }

[dev-dependencies]
itertools = "0.10.0"
ndarray-npy = "0.7.1"
# widely popular base64 decoding/encoding crate
base64 = "0.11.0"
...
mod tests {
    ...
    fn test_predictions() {
        let (features, expected_predictions): (Array2<usize>, Array1<f32>) = {
            let mut npz =
                NpzReader::new(File::open("testdata/features.npz").unwrap()).unwrap();
        ...
        for (fv, expected_dirty_score) in izip!(features.axis_iter(Axis(0)),
                                               expected_predictions.iter()){
            ...
            assert!(
                ...
                format!(
                    ...
                    dirty_score, expected_dirty_score
                )
            );
...
lazy_static! {{
    static ref MODEL: {class_name} =
        bincode::deserialize(raw_model::raw_model()).unwrap();
    static ref THRESHOLDS: BinaryClassThresholds =
        serde_json::from_str(include_str!("../model/thresholds.json")).unwrap();
}}
...
mod raw_model {{
    ...
    pub(crate) fn raw_model() -> &'static [u8] {{
        ...
        // have to do some pointer shenanigans to find it, then the `ld` invocation prepends
    ...
    pub(crate) fn raw_model() -> &'static [u8] {{
        ...
        let name_id = PCWSTR(
            NAME_ID_MODEL_FILE
                .encode_utf16()
                // And always remember that Windows strings have to be null terminated...
...
lazy_static! {
    static ref FEATURES: Array2<usize> = {
    ...
    c.bench_function(
        ...
        |b| {
            b.iter_custom(|iters| {
                ...
                for _i in 0..iters {
                    duration.push(
                        head_features.axis_iter(Axis(0))
                        .map(|fv| {
                                let start = Instant::now();
                                let fv_to_vec = fv.into_owned().into_raw_vec();
